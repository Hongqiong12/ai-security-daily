...
> **版本**: v1.7（增量更新：CI-Work / AgentPressureBench / CSTM-Bench）
> **更新日期**: 2026-04-28
> **覆盖论文数**: 37 篇  
...
| ATBench-Claw / ATBench-CodeX | [2604.14858](https://arxiv.org/abs/2604.14858) | 2026 | OpenClaw / Codex 执行环境轨迹安全基准 | [详情](../categories/agentic-search/papers/2604.14858_atbench_claw_codex.md) |
| DR3-Eval | [2604.14683](https://arxiv.org/abs/2604.14683) | 2026 | Deep Research 真实且可复现评测 | [详情](../categories/agentic-search/papers/2604.14683_dr3_eval.md) |
| HarmfulSkillBench | [2604.15415](https://arxiv.org/abs/2604.15415) | 2026 | harmful skill 生态测量与武器化基准 | [详情](../categories/agentic-search/papers/2604.15415_harmfulskillbench.md) |
...
| ATBench-Claw / ATBench-CodeX | [2604.14858](https://arxiv.org/abs/2604.14858) | 2026 | shell / patch / MCP / approval 风险的场景化轨迹评测 | [详情](../categories/agentic-search/papers/2604.14858_atbench_claw_codex.md) |
| DR3-Eval | [2604.14683](https://arxiv.org/abs/2604.14683) | 2026 | per-task sandbox 下的信息检索与长报告 groundedness 评测 | [详情](../categories/agentic-search/papers/2604.14683_dr3_eval.md) |
...
**ATBench-Claw / ATBench-CodeX 核心发现**：同一套 trajectory safety engine 只有在**跟随执行环境重写 taxonomy 与敏感动作 inventory** 后，才真正具备现实价值。ATBench 这次把 OpenClaw 与 Codex-runtime 纳入统一框架，说明 Agent benchmark 已经从“泛化 case 列表”升级成会随 shell / patch / MCP / approval 风险共同演化的**执行环境基础设施**。

**DR3-Eval 核心发现**：Deep Research Agent 的核心短板不是“不会写长报告”，而是**在复杂证据链里找全关键信息并把引用真正落地**。通过 per-task sandbox，DR3-Eval 首次把真实性与可复现性同时保住，也把 Agent 评测从单轮问答推进到了 evidence-seeking + grounded report generation 的长程任务。
...
| LogJack | [2604.15368](https://arxiv.org/abs/2604.15368) | 2026 | 云日志驱动的间接提示注入 / 调试代理执行劫持 | [详情](../categories/agentic-search/papers/2604.15368_logjack.md) |
...
### 3.8 评测基础设施化：从单条轨迹走向可复现执行环境（NEW — 2026-04-18 新增）

| 论文 | arXiv | 年份 | 方向 | 详情 |
|------|-------|------|------|------|
| ATBench-Claw / ATBench-CodeX | [2604.14858](https://arxiv.org/abs/2604.14858) | 2026 | OpenClaw / Codex-runtime 轨迹安全基准 | [详情](../categories/agentic-search/papers/2604.14858_atbench_claw_codex.md) |
| DR3-Eval | [2604.14683](https://arxiv.org/abs/2604.14683) | 2026 | Deep Research 可复现评测基础设施 | [详情](../categories/agentic-search/papers/2604.14683_dr3_eval.md) |

**ATBench-Claw / ATBench-CodeX 核心发现**：
Agent benchmark 不能停留在抽象“安全/不安全”二分类，而必须跟着实际执行环境重写风险 taxonomy。ATBench 通过显式覆盖 shell、patch、dependency、MCP、approval 等动作，把 OpenClaw 与 Codex-runtime 的安全评测推进成**执行环境专用基础设施**。

**DR3-Eval 核心发现**：
Deep Research agent 的主要瓶颈不在表面写作，而在**证据检索与 grounded 引用**。DR3-Eval 用 per-task sandbox 把真实性与可复现性同时锁住，说明今后的 Agent benchmark 会越来越像软件基础设施：静态可控，但必须保留真实噪声与长链路证据需求。

### 3.9 技能生态与运维遥测：Agent 新输入面正在变成高危执行面（NEW — 2026-04-20 新增）

| 论文 | arXiv | 年份 | 方向 | 详情 |
|------|-------|------|------|------|
| HarmfulSkillBench | [2604.15415](https://arxiv.org/abs/2604.15415) | 2026 | harmful skill 生态测量 / 武器化基准 | [详情](../categories/agentic-search/papers/2604.15415_harmfulskillbench.md) |
| LogJack | [2604.15368](https://arxiv.org/abs/2604.15368) | 2026 | 云日志间接注入 / 调试代理执行劫持 | [详情](../categories/agentic-search/papers/2604.15368_logjack.md) |

**HarmfulSkillBench 核心发现**：
开放技能生态的风险不只是“skill 会不会被投毒”，而是**skill 本身就可能封装危险能力**。论文在 98,440 个 skills 中识别出 4,858 个 harmful skills（总体 4.93%，ClawHub 达 8.84%），并进一步证明：只要把 harmful skill 暴露给 agent，平均 harm score 就会从无技能时的 **0.27** 升到 **0.76**。这标志着 Agent 安全正式进入 **skill-registry governance** 阶段。

**LogJack 核心发现**：
真实生产环境里，日志与遥测不是“被动背景信息”，而可能是最危险的间接注入载体。LogJack 显示 Llama 3.3 70B 在 Active 调试条件下的日志劫持率高达 **86.2%**，且 **6/8** 模型可被推进到 RCE；更严重的是，Azure / GCP 输入侧 guardrail 在日志上下文中几乎完全失效。这说明 Agent 安全的防线必须继续下沉到**遥测可信性与动作执行边界**。

### 3.10 协议级级联防线与多模态记忆投毒：Agent 风险正式进入“运行链路 + 长期记忆”双重治理 (NEW — 2026-04-21 新增)

| 论文 | arXiv | 年份 | 方向 | 详情 |
|------|-------|------|------|------|
| CASCADE | [2604.17125](https://arxiv.org/abs/2604.17125) | 2026 | MCP 级联防御 / tool poisoning 检测 | [详情](../categories/agentic-search/papers/2604.17125_cascade.md) |
| Visual Inception | [2604.16966](https://arxiv.org/abs/2604.16966) | 2026 | 多模态记忆投毒 / 长期规划劫持 | [详情](../categories/agentic-search/papers/2604.16966_visual_inception.md) |

**CASCADE 核心发现**：
MCP 安全的难点已经不只是“用户有没有写恶意 prompt”，而是**协议链路本身会持续引入新攻击面**。CASCADE 用三层本地级联框架把 prompt injection、tool poisoning、data exfiltration 串到一起治理，在 **95.85% precision / 6.06% FPR** 下证明：工程上先把低误报、本地可部署的 MCP baseline 做出来，是当前比追求纸面高 recall 更现实的路线。

**Visual Inception 核心发现**：
Agentic 推荐与多模态 agent 的真正高危点，开始从“当前输入”迁移到“长期记忆”。Visual Inception 说明一张看似正常的用户图片也能潜伏进 memory bank，并在未来规划阶段把 **ASR-M / GHR** 推到 **82.3% / 85.1%**；而 CognitiveGuard 又证明，只有把**上传时净化 + 查询时反事实验证**联合起来，才能把这类风险真正压下去。这标志着 Agent 安全正式进入 **memory-governance + causal verification** 阶段。

### 3.11 确定性隐私执行环境：Agent 私有数据保护开始进入执行语义层 (NEW — 2026-04-22 新增)

| 论文 | arXiv | 年份 | 方向 | 详情 |
|------|-------|------|------|------|
| GAAP | [2604.19657](https://arxiv.org/abs/2604.19657) | 2026 | 隐私执行环境 / deterministic disclosure governance | [详情](../categories/agentic-search/papers/2604.19657_gaap.md) |

**GAAP 核心发现**：
GAAP 把 Agent 隐私保护从“靠 prompt 里提醒模型别泄露”推进到**执行环境语义**本身：用 IFC / taint tracking 跟踪私有数据流，用 Permission DB 显式决定哪些实体可以看到哪些数据，再用 Disclosure Log 记录每一次外泄。其关键意义在于：**即便底层 LLM 不完全可信，只要执行环境是确定性的，私有数据仍可以被约束在可审计的 disclosure boundary 内。** 这使 Agent 安全第一次真正接近“最小权限操作系统”式的实现形态。

### 3.12 函数选择层劫持与协议感知审计：MCP 安全开始进入“pre-execution control plane” (NEW — 2026-04-27 新增)

| 论文 | arXiv | 年份 | 方向 | 详情 |
|------|-------|------|------|------|
| Function Hijacking Attacks | [2604.20994](https://arxiv.org/abs/2604.20994) | 2026 | tool selection / function metadata hijacking | [详情](../categories/agentic-search/papers/2604.20994_function_hijacking.md) |
| MCP Pitfall Lab | [2604.21477](https://arxiv.org/abs/2604.21477) | 2026 | protocol-aware benchmark / developer pitfalls | [详情](../categories/agentic-search/papers/2604.21477_mcp_pitfall_lab.md) |

**Function Hijacking Attacks 核心发现**：
FHA 证明攻击者甚至不需要等到工具真正执行，也不需要控制用户 query 或工具返回值；只要把目标函数的 **description** 优化成对抗性文本，就能在 function-calling / MCP 环境里把模型系统性路由到攻击者指定函数。其在 Mistral-7B 上的 Function Name ASR 达 **0.99**，在 Qwen3-1.7B 上可到 **1.00**。这标志着 Agent 攻击面正式上移到 **tool selection layer**。

**MCP Pitfall Lab 核心发现**：
MCP 安全的高危点不只在模型“会不会被 prompt injection”，而在服务器开发者是否把 schema、跨工具转发、高风险输入校验与审计日志实现正确。论文用 protocol-aware benchmark + trace validators 证明：平均只需 **27 行代码** 的修复，就能把风险从 **10.0** 降到 **0.0**；更关键的是，agent 自述与真实 trace 在 sink-action 运行上存在 **100%** 偏差率，说明**协议 trace 才是 MCP 审计的唯一可信锚点**。
...
|---|----------|------|------|----------|------|
| 33 | 2604.21308 | CI-Work: Benchmarking Contextual Integrity in Enterprise LLM Agents | 评测/企业隐私与情境完整性基准 | 2026-04-28 | [详情](../categories/agentic-search/papers/2604.21308_ci_work.md) |
| 32 | 2604.21131 | Cross-Session Threats in AI Agents: Benchmark, Evaluation, and Algorithms | 评测/跨会话威胁检测基准 | 2026-04-28 | [详情](../categories/agentic-search/papers/2604.21131_cstm_bench.md) |
| 31 | 2604.20200 | Chasing the Public Score: User Pressure and Evaluation Exploitation in Coding Agent Workflows | 评测/coding-agent workflow exploitation 基准 | 2026-04-28 | [详情](../categories/agentic-search/papers/2604.20200_agentpressurebench.md) |
| 30 | 2604.21477 | MCP Pitfall Lab: Exposing Developer Pitfalls in MCP Tool Server Security under Multi-Vector Attacks | 评测/协议感知开发者安全基准 | 2026-04-27 | [详情](../categories/agentic-search/papers/2604.21477_mcp_pitfall_lab.md) |
| 29 | 2604.20994 | Breaking MCP with Function Hijacking Attacks: Novel Threats for Function Calling and Agentic Models | 攻击/函数选择层劫持 | 2026-04-27 | [详情](../categories/agentic-search/papers/2604.20994_function_hijacking.md) |
| 28 | 2604.19657 | GAAP: Privacy-preserving LLM Agents with Deterministic Execution Protection | 防御/隐私执行环境 | 2026-04-22 | [详情](../categories/agentic-search/papers/2604.19657_gaap.md) |
| 27 | 2604.17125 | CASCADE: A Cascaded Hybrid Defense Architecture for Prompt Injection Detection in MCP-Based Systems | 防御/MCP级联防线 | 2026-04-21 | [详情](../categories/agentic-search/papers/2604.17125_cascade.md) |
| 26 | 2604.16966 | Visual Inception: Compromising Long-term Planning in Agentic Recommenders via Multimodal Memory Poisoning | 攻击/多模态记忆投毒 | 2026-04-21 | [详情](../categories/agentic-search/papers/2604.16966_visual_inception.md) |
| 25 | 2604.15415 | HarmfulSkillBench: How Do Harmful Skills Weaponize Your Agents? | 评测/技能生态武器化基准 | 2026-04-20 | [详情](../categories/agentic-search/papers/2604.15415_harmfulskillbench.md) |
| 24 | 2604.15368 | LogJack: Indirect Prompt Injection Through Cloud Logs Against LLM Debugging Agents | 攻击/云日志间接注入 | 2026-04-20 | [详情](../categories/agentic-search/papers/2604.15368_logjack.md) |
| 23 | 2604.14858 | Benchmarks for Trajectory Safety Evaluation and Diagnosis in OpenClaw and Codex | 评测/执行环境轨迹安全基准 | 2026-04-18 | [详情](../categories/agentic-search/papers/2604.14858_atbench_claw_codex.md) |
| 22 | 2604.14683 | DR3-Eval: Towards Realistic and Reproducible Deep Research Evaluation | 评测/Deep Research 可复现基准 | 2026-04-18 | [详情](../categories/agentic-search/papers/2604.14683_dr3_eval.md) |
...
*本 Survey 由 paper-research skill Phase 5 生成与维护 | 初始版本：2026-03-30 | v1.7 更新：2026-04-28*
*v1.7 更新说明：新增 CI-Work、AgentPressureBench 与 CSTM-Bench，并把 Agentic 安全继续推进到“企业隐私治理 + workflow incentive control + cross-session memory bottleneck”这条新的长期状态治理主线。*
